...
export function determineAnimeSeason(month: number, year: number): number {
  return (
    determineAnimeSeasonMonth(month) +
    (4 * determineAnimeSeasonYear(year, month) - baseYear)
  );
}

function determineAnimeSeasonMonth(month: number): number {
  if (month >= 3 && month <= 5) {
    // квітень, травень, червень - весна
    return 1;
  } else if (month >= 6 && month <= 8) {
    // липень, серпень, вересень - літо
    return 2;
  } else if (month >= 9 && month <= 11) {
    // жовтень, листопад, грудень - осінь
    return 3;
  } else {
    // січень, лютий, березень - зима
    return 4;
  }
}
